...
from app.forms import ResetPasswordRequestForm, CheckMasterForm, AddScheduleForm, ConfirmForm
...
def index():
    ...
    return render_template('index.html', title=_('Home'),
                           masters=masters.items, users = users, next_url=next_url,
                           prev_url=prev_url)
...
@app.route('/schedule/<master_id>', methods=['GET', 'POST'])
@login_required
def schedule(master_id):
    master = Master.query.filter_by(id=master_id).first_or_404()
    page = request.args.get('page', 1, type=int)
    variants = master.variants.filter_by(choosen=False).order_by(Shedule.date_time.asc()).paginate(
        page, app.config['POSTS_PER_PAGE'], False)
    ...
    return render_template('schedule.html', username=master.master, variants=variants.items,
                           next_url=next_url, prev_url=prev_url)
...
def actual_schedules(username):
    ...
    variants = user.sh_vars.order_by(Shedule.date_time.asc()).paginate(
        page, app.config['POSTS_PER_PAGE'], False)
    next_url = url_for('actual_schedules', username=user.username, page=variants.next_num) \
        if variants.has_next else None
    prev_url = url_for('actual_schedules', username=user.username, page=variants.prev_num) \
        if variants.has_prev else None
    return render_template('actual_schedules.html', title=_('Actual schedules'), 
                           user = user, variants=variants.items,
                           next_url=next_url, prev_url=prev_url)
...
def add_variant(master_id):
    ...
    if form.validate_on_submit():
        ...
        if int(hour) < 7 or int(hour) > 23:
            ...
            return redirect(url_for('user', username=master.master))
        try:
            date_time = datetime(int(year), int(month), int(day), int(hour)-3, int(minute))
        except ValueError:
            flash(_('Date and time are incorrect.'))
            return redirect(url_for('user', username=master.master))
        s_check = Shedule.query.filter_by(master_id = master_id, date_time = date_time).first()
        if not s_check:
            s = Shedule(master_id = master_id, date_time = date_time, choosen = False)
            db.session.add(s)
            db.session.commit()
        ...
        return redirect(url_for('user', username=master.master))
    return render_template('add_variant.html', title=_('Add Schedule variant'), form=form)
...
def check_master():  
    ...
    return render_template('check_master.html', title=_('Check master'), form=form)
...
def choose(variant): 
    ...
    form = ConfirmForm()
    if form.validate_on_submit():
        s = Shedule.query.filter_by(master_id = master_id, date_time = date_time).first()
        s.choosen = True
    ...
    return render_template('confirm_variant.html', title=_('Confirm variant'), 
                            master = master_chosen, time = date_time, form = form)
